...
PLAYER_HEALTH_LEVEL_UP_BONUS = 20 # Эта константа больше не используется для увеличения max_health
...
def _play_sound(sound_obj):
    ...
    if mixer_available and sound_obj: # Добавлена проверка sound_obj на None
        sound_obj.play()
...
def load_image(path, size=None, colorkey=None):
    ...
    try:
        ...
        # Возвращаем пустую поверхность, чтобы избежать ошибок в дальнейшем коде
...
class Player:
    ...
    def level_up(self):
        ...
        # self.max_health += PLAYER_HEALTH_LEVEL_UP_BONUS # Удалено: максимальное здоровье больше не увеличивается
        # self.health = self.max_health # Удалено: здоровье больше не восстанавливается при повышении уровня
        self.damage += PLAYER_DAMAGE_LEVEL_UP_BONUS
        self.speed += PLAYER_SPEED_LEVEL_UP_BONUS

    def update(self, dt, obstacles, enemies): # Добавлен enemies для поворота спрайта
...
class Game:
    ...
    def draw(self):
        ...
        if self.settings_visible:
            ...
            screen.blit(self.title_image, title_rect) # Рисуем заголовок
            mouse_pos = pygame.mouse.get_pos()
            ...
            # _draw_overlay(screen) # Оверлей убран, чтобы фон был полностью виден
            title = self.font.render("Select Character", True, WHITE) # Используем self.font
            screen.blit(title, (WIDTH//2 - title.get_width()//2, HEIGHT//4))
            mouse_pos = pygame.mouse.get_pos()
            ...
            strong_desc = self.font.render("High damage, slow attack", True, BLUE) # Используем self.font
            fast_desc = self.font.render("Low damage fast attack", True, GREEN) # Используем self.font
            ...
            for enemy in self.enemies:
                enemy.draw(screen)
            # Отрисовка HUD
            health_text = self.font.render(f"HP: {self.player.health}/{self.player.max_health}", True, WHITE) # Используем self.font
            level_text = self.font.render(f"Level: {self.player.level}", True, WHITE) # Используем self.font
            wave_text = self.font.render(f"Wave: {self.wave}/{self.max_waves}", True, WHITE) # Используем self.font
            score_text = self.font.render(f"Score: {self.score}", True, WHITE) # Используем self.font
            ...
            title = self.font.render("Game Paused", True, WHITE) # Используем self.font
            screen.blit(title, (WIDTH//2 - title.get_width()//2, HEIGHT//4))
            mouse_pos = pygame.mouse.get_pos()
            ...
            _draw_overlay(screen)
            title = self.big_font.render("Game Over", True, RED) # Используем self.big_font
            screen.blit(title, (WIDTH//2 - title.get_width()//2, HEIGHT//4))
            ...
            _draw_overlay(screen)
            title = self.big_font.render("Victory!", True, GREEN) # Используем self.big_font
            screen.blit(title, (WIDTH//2 - title.get_width()//2, HEIGHT//4))
            self._draw_game_stats(screen, offset_y=HEIGHT//4) # Используем смещение для статистики
            mouse_pos = pygame.mouse.get_pos()
            self.restart_button.check_hover(mouse_pos)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.settings_visible:
                    ...
                    if self.restart_button.is_clicked(mouse_pos, event):
                        _play_sound(button_sound)
                        self.start_game(self.player.type) # Перезапуск с тем же типом игрока
                    elif self.menu_button.is_clicked(mouse_pos, event):
                        _play_sound(button_sound)
                        self.state = "menu"
        return True
